...
const Tooltip = ({ text }: { text: string }) => (
  <div className="group relative inline-block ml-2">
    <HelpCircle
      size={16}
      className="text-slate-400 hover:text-red-600 cursor-help"
    />
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div
            className={`w-12 h-12 rounded-xl flex items-center justify-center shadow-sm transition-all duration-300 ${expanded ? 'bg-red-600 shadow-red-200' : 'bg-slate-200'}`}
          >
            <Icon
              size={24}
              className={expanded ? 'text-white' : 'text-slate-500'}
            />
          </div>
          <div>
            <h3
              className={`text-lg font-black ${expanded ? 'text-red-700' : 'text-slate-700'}`}
            >
              {title}
            </h3>
...
export const LandingPageSettings: React.FC<LandingPageSettingsProps> = ({
  ...
  const handleGalleryUpload = async (
    ...
    try {
      ...
      setConfig({
        ...config,
        gallery_images: [...currentImages, ...newImages],
      });
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Personalize o conteúdo da sua Landing Page
          </p>
        </div>
        {isAdmin && (
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-blue-700">
              ...
              Edite apenas as seções **Hero**, **Sobre**, **Serviços** e
              **Contato**.
            </p>
          </div>
        </div>
      ...
      >
        <Input
          label={<span className="flex items-center">Título Principal</span>}
        ...
        <Textarea
          label={<span className="flex items-center">Subtítulo / Slogan</span>}
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <Input
            label={<span className="flex items-center">Texto do Badge</span>}
          ...
          <Input
            ...
            onChange={(e) =>
              setConfig({ ...config, cta_button_text: e.target.value })
            }
      ...
      >
        <Input
          ...
          onChange={(e) =>
            setConfig({ ...config, about_title: e.target.value })
          }
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <Textarea
            ...
            onChange={(e) =>
              setConfig({ ...config, mission_text: e.target.value })
            }
            rows={3}
          ...
          <Textarea
            ...
            onChange={(e) =>
              setConfig({ ...config, vision_text: e.target.value })
            }
            rows={3}
          ...
          <Textarea
            ...
            onChange={(e) =>
              setConfig({ ...config, values_text: e.target.value })
            }
            rows={3}
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <p className="text-sm text-slate-600">
            Recomendado: 4 serviços principais
          </p>
          {isAdmin && (
            <Button
              onClick={addService}
        ...
        {config.services && config.services.length > 0 ? (
          <div className="space-y-3">
            {config.services.map((service, index) => (
              <div
                key={index}
                className="bg-slate-50 p-4 rounded-lg border border-slate-200 flex flex-col md:flex-row gap-4 items-start"
              >
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            {config.gallery_images?.map((img, idx) => (
              <div
                key={idx}
                className="relative group aspect-video bg-slate-100 rounded-lg overflow-hidden border border-slate-200"
              >
                <img
                  src={img}
                  alt={`Galeria ${idx}`}
          ...
          <p className="text-xs text-slate-500">
            ...
            Formatos aceitos: JPG, PNG. Recomendado: 1200x500px (Formato
            Horizontal/Banner).
          </p>
        </div>
      </SectionCard>
