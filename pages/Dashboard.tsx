...
export const Dashboard: React.FC<DashboardProps> = ({ user }) => {
  ...
  const totalBalance = completedTx.reduce(
    ...
    return (
      <div className="space-y-6 animate-in fade-in duration-500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map((i) => (
            <StatCard key={i} title="" value="" icon={null} loading />
          ))}
        </div>
      </div>
    );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Bem-vindo ao sistema de gestão integrada.
          </p>
        </div>
        ...
        <StatCard
          ...
          trend={
            criticalItemsCount > 0
              ? `${criticalItemsCount} itens sem estoque`
              : 'Estoque regular'
          }
          trendUp={false}
        />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
        {shortcuts.map((item, index) => {
          ...
          return (
            ...
            >
              <div
                className={`w-12 h-12 rounded-lg ${item.bg} ${item.color} flex items-center justify-center mb-3`}
              >
                <Icon size={24} />
              </div>
              ...
              recentTransactions.map((tx) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div
                      className={`w-10 h-10 rounded-xl flex items-center justify-center transition-transform group-hover:scale-110 ${tx.type === 'INCOME' ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-600'}`}
                    >
                      {tx.type === 'INCOME' ? (
                  ...
                  <div className="text-right">
                    <p
                      className={`text-sm font-black ${tx.type === 'INCOME' ? 'text-emerald-600' : 'text-slate-900'}`}
                    >
                      {tx.type === 'EXPENSE' ? '- ' : ''}
