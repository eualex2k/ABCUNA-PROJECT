      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Gerencie os tokens de registro e permissões do sistema.
          </p>
        </div>
        {isAdmin && (
          <Button
          ...
          >
            ...
            <div>
              ...
              <select
                ...
                value={newCode.role}
                onChange={(e) =>
                  setNewCode({ ...newCode, role: e.target.value as UserRole })
                }
              >
      ...
      <div className="grid grid-cols-1 gap-4">
        {codes.map((code) => {
          ...
          return (
            <Card
              key={code.id}
              className={`p-0 overflow-hidden ${!code.active ? 'opacity-75 grayscale-[0.5]' : ''}`}
            >
                ...
                <div className="p-6 flex-1 w-full">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <h3 className="font-mono font-bold text-lg text-slate-900 tracking-wider">
                            {code.code}
                          </h3>
                          <button
                        ...
                        <p className="text-sm text-slate-500">
                          {code.description || 'Sem descrição'}
                        </p>
                      </div>
                    </div>
                    <span
                      className={`px-2 py-1 rounded text-xs font-semibold border ${getRoleColor(code.role)}`}
                    >
                  ...
                  <div className="mt-4">
                    <div className="flex justify-between text-xs mb-1 font-medium">
                      ...
                      <span
                        className={`${isLimitReached ? 'text-red-600' : 'text-brand-600'}`}
                      >
                    ...
                    <div className="w-full bg-slate-100 rounded-full h-2">
                      <div
                        ...
                        style={{
                          width: `${code.limit === null ? 100 : Math.min(usagePercent, 100)}%`,
                        }}
                      ></div>
                    </div>
                  </div>
                ...
                {isAdmin && (
                  <div className="bg-slate-50 border-t md:border-t-0 md:border-l border-slate-100 p-4 md:h-full flex md:flex-col gap-2 justify-center w-full md:w-auto min-w-[140px]">
                    ...
                    >
                      {code.active ? (
                        <>
                          <Power size={14} className="mr-2" /> Ativo
                        </>
                      ) : (
                        <>
                          <AlertCircle size={14} className="mr-2" /> Inativo
                        </>
                      )}
                    </Button>
