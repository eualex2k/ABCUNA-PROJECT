...
export const AuditPage: React.FC<AuditPageProps> = ({ user }) => {
  ...
  const getCategoryIcon = (cat: DocCategory) => {
    switch (cat) {
      case 'ESTATUTO':
        return <Scale size={18} className="text-indigo-600" />;
      case 'REGIMENTO':
  ...
  const getCategoryBadge = (cat: DocCategory) => {
    switch (cat) {
      case 'ESTATUTO':
        return <Badge variant="neutral">Jurídico</Badge>;
      case 'REGIMENTO':
        return <Badge variant="info">Normativo</Badge>;
      case 'ATA':
        return <Badge variant="neutral">Registro</Badge>;
      case 'EDITAL':
        return <Badge variant="warning">Público</Badge>;
      case 'FINANCEIRO':
        return <Badge variant="success">Contábil</Badge>;
    }
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            Repositório oficial de estatutos, atas, editais e regimentos da
            associação.
          </p>
        </div>
        {isAdminOrSec && (
      ...
      >
        <form onSubmit={handleUpload} className="space-y-4">
          ...
          <div
            className="border-2 border-dashed border-slate-300 rounded-lg p-6 text-center text-slate-500 hover:bg-slate-50 transition-colors cursor-pointer"
