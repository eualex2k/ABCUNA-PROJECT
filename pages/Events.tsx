...
export const EventsPage = React.forwardRef<EventsPageRef, EventsPageProps>(
  ({ user, initialView = 'list' }, ref) => {
    const canEdit = [
      UserRole.ADMIN,
      UserRole.SECRETARY,
      UserRole.INSTRUCTOR,
    ...
    const [formEvent, setFormEvent] =
      useState<Partial<Event>>(initialFormState);
    ...
    return (
      <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <Card className="xl:col-span-3 p-0 border-slate-200 overflow-hidden shadow-xl shadow-slate-100 rounded-[2.5rem]">
              <div className="bg-slate-900 text-white p-8 flex justify-between items-center">
                <div>
                  ...
                  <p className="text-brand-400 font-black text-sm uppercase tracking-[0.3em] ml-0.5">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="w-12 h-12 flex items-center justify-center bg-white/10 hover:bg-white/20 rounded-2xl transition-all border border-white/5"
                  >
                    <ChevronLeft size={24} />
                  </button>
                  <button
              ...
              <div className="p-8">
                ...
                <div className="grid grid-cols-7 gap-4">
                  {Array.from({
                    ...
                    <div
                      key={`empty-${i}`}
                      className="h-28 rounded-2xl bg-slate-50 opacity-30"
                    />
                  ))}
                  ...
                  {Array.from({
                    ...
                    return (
                      <div
                        key={day}
                        className={`h-28 rounded-3xl p-3 border group transition-all relative ${isToday ? 'bg-brand-50 border-brand-200 shadow-inner' : 'bg-white border-slate-100 hover:border-slate-300 hover:shadow-md'}`}
                      >
                        <span
                          className={`text-lg font-black ${isToday ? 'text-brand-600' : 'text-slate-400 group-hover:text-slate-900'}`}
                        >
                          {day}
                        </span>
                        ...
                        <div className="mt-2 space-y-1 overflow-hidden">
                          {dayHolidays.map((h, idx) => (
                            <div
                              key={idx}
                              className="bg-amber-100 text-amber-700 text-[8px] font-black uppercase px-2 py-0.5 rounded-full truncate border border-amber-200"
                            >
                              {h.title}
                            </div>
                          ))}
                ...
                <div className="space-y-4">
                  ...
                  {holidays
                    ...
                    .map((h, idx) => (
                      <div
                        key={idx}
                        className="flex items-center gap-3 p-3 bg-amber-50 rounded-2xl border border-amber-100 shadow-sm"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-[10px] font-black text-amber-700 uppercase tracking-widest line-clamp-1">
                            {h.title}
                          </p>
                          <p className="text-[8px] font-bold text-amber-600/70 uppercase tracking-widest">
                            Feriado Nacional
                          </p>
                        </div>
                      </div>
                  ...
                  holidays.filter((h) => {
                    ...
                    events
                      ...
                      .map((e, idx) => (
                        ...
                        >
                          <div
                            className={`w-10 h-10 text-white rounded-xl flex flex-col items-center justify-center transition-transform group-hover:scale-110 ${e.visibility === 'BOARD' ? 'bg-blue-600 shadow-lg shadow-blue-100' : 'bg-emerald-600 shadow-lg shadow-emerald-100'}`}
                          >
                            <span className="text-xs font-black">
                              {e.date.split('-')[2]}
                            </span>
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-[10px] font-black text-slate-900 uppercase tracking-widest line-clamp-1 leading-tight mb-0.5 group-hover:text-brand-600 transition-colors">
                              {e.title}
                            </p>
              ...
              <div className="p-6 bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl shadow-slate-200 relative overflow-hidden group">
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-slate-100 text-[10px] font-bold uppercase tracking-widest leading-relaxed opacity-90">
                    Ajude a planejar as atividades da associação sugerindo
                    eventos à diretoria.
                  </p>
                </div>
              </div>
          ...
          <div className="grid gap-6">
            {loading ? (
              <div className="py-20 text-center text-slate-400 italic">
                Buscando agenda atualizada...
              </div>
            ) : (
              visibleEvents.map((event) => (
                <Card
                  key={event.id}
                  className={`p-0 overflow-hidden hover:shadow-md transition-all border-slate-200 group ${event.status === 'FINISHED' ? 'opacity-75 bg-slate-50/50' : ''}`}
                >
                  <div className="flex flex-col md:flex-row">
                    <div
                      className={`md:w-48 border-r border-slate-100 p-6 flex flex-col items-center justify-center text-center ${event.status === 'FINISHED' ? 'bg-slate-100' : 'bg-slate-50'}`}
                    >
                      <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">
                        Data
                      </p>
                      <p
                        className={`text-3xl font-black leading-none ${event.status === 'FINISHED' ? 'text-slate-400' : 'text-brand-600'}`}
                      >
                      ...
                      <Badge
                        variant="neutral"
                        className="mt-3 text-[10px] font-bold bg-white"
                      >
                        {event.time}
                      </Badge>
                    </div>
                    ...
                    <div className="flex-1 p-6 space-y-4">
                      <div className="flex justify-between items-start">
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            <h3
                              className={`text-xl font-black transition-colors ${event.status === 'FINISHED' ? 'text-slate-500 line-through' : 'text-slate-900 group-hover:text-brand-600'}`}
                            >
                              {event.title}
                            </h3>
                            ...
                            {event.visibility === 'BOARD' && (
                              <Badge
                                variant="info"
                                className="flex items-center gap-1 bg-blue-50 text-blue-600 border-blue-100"
                              >
                        ...
                        {canEdit && (
                          <div className="flex gap-2">
                            ...
                            <Button
                              variant="ghost"
                              size="sm"
                              className="text-slate-400 hover:text-red-500 h-9"
                      ...
                      <div className="flex flex-wrap gap-4 pt-2">
                        ...
                        <div
                          className={`flex items-center gap-2 text-xs font-bold px-3 py-1.5 rounded-lg border ${event.visibility === 'BOARD' ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-emerald-50 text-emerald-600 border-emerald-100'}`}
                        >
                          {event.visibility === 'BOARD' ? (
                            <>
        ...
        <Modal
          ...
          title={editingId ? 'Editar Evento' : 'Agendar Novo Evento'}
        >
          <form onSubmit={handleSaveEvent} className="space-y-4">
            <Input
              label="Título do Evento"
              placeholder="Ex: Treinamento NR35"
