...
export const SchedulePage = React.forwardRef<
  SchedulePageRef,
  SchedulePageProps
>(({ user }, ref) => {
  ...
  const handleCreateShift = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const initialStatus = newShift.leader
      ...
      alert(
        newShift.leader
          ? 'Plantão enviado para o líder confirmar!'
          : 'Plantão lançado com sucesso!'
      );
    } catch (error) {
  ...
  const handleUpdateShift = async (e: React.FormEvent) => {
    ...
    if (editingShift && editingShift.id) {
      try {
        ...
        const leaderProfile = directors.find(
          (d) => d.name === updatedShift.leader
        );

        if (
          leaderProfile &&
          !updatedShift.members?.some((m) => m.userId === leaderProfile.id)
        ) {
  ...
  const handleConfirmScale = async () => {
    if (previewShiftId && previewMembers.length > 0) {
      try {
        ...
        await scheduleService.update(previewShiftId, {
  ...
  const handleVolunteer = async (shift: Shift) => {
    if (
      confirm(
        `Solicitar inscrição no plantão "${shift.team}"? Sua solicitação precisará ser aprovada.`
      )
    ) {
      try {
        await scheduleService.volunteer(
          shift.id,
          user.id,
          user.name,
          user.avatar
  ...
  const handleRejectVolunteer = async (shift: Shift, memberId: string) => {
    ...
    if (confirm('Rejeitar este voluntário?')) {
      try {
        const updatedMembers = shift.members.filter(
          (m) => m.userId !== memberId
        );
        await scheduleService.updateMembers(shift.id, updatedMembers);
    ...
    try {
      ...
      const isLeader =
        ...
        user.name.trim().toLowerCase() ===
          (selectedShift.leader || '').trim().toLowerCase();

      if (!accept && isLeader) {
        const otherDirectors = directors.filter(
          (d) => d.name.trim().toLowerCase() !== user.name.trim().toLowerCase()
        );
        if (otherDirectors.length > 0) {
          const nextLeader =
            otherDirectors[Math.floor(Math.random() * otherDirectors.length)]; // Seleção aleatória entre outros diretores
          ...
          alert(
            `Você recusou a liderança. O diretor ${nextLeader.name} foi notificado para assumir.`
          );
        } else {
          await scheduleService.update(shiftId, { leader: '' });
          alert(
            'Você recusou a liderança. Sem outros diretores, o cargo ficou vago.'
          );
        }
        ...
        const hasMembers =
          selectedShift.members.filter((m) => m.status === 'CONFIRMED').length >
          0;
        const isFull =
          selectedShift.members.filter((m) => m.status === 'CONFIRMED')
            .length >= selectedShift.vacancies;
        ...
        alert(
          'Liderança confirmada! O plantão está pronto para a escala da equipe.'
        );
      } else {
        alert(
          accept ? 'Escala confirmada com sucesso!' : 'Convocação recusada.'
        );
      }
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 scrollbar-hide">
        {shifts.map((shift) => (
          <Card
            key={shift.id}
            className="p-0 overflow-hidden hover:shadow-2xl transition-all duration-300 border-slate-200 group flex flex-col h-full bg-white rounded-3xl"
          >
            ...
            <div className="p-6 pb-0 flex items-start justify-between">
              <div className="flex gap-4 items-center">
                ...
                <div>
                  <h3 className="font-black text-slate-900 text-2xl uppercase leading-none tracking-tighter mb-1">
                    {shift.team}
                  </h3>
              ...
              <div className="flex flex-col items-end gap-2">
                <Badge
                  ...
                  className="px-4 py-1.5 font-black text-[9px] uppercase tracking-widest border-none shadow-sm"
                >
                ...
                {canEdit && (
                  <div className="flex gap-2">
                    <button
                      onClick={() => {
                        setEditingShift({ ...shift, date: shift.fullDate });
                        setIsEditModalOpen(true);
                      }}
                      className="hover:scale-110 transition-transform text-slate-300 hover:text-brand-600"
                    >
                      <Edit size={16} />
                    </button>
                    <button
                      onClick={() => handleDeleteShift(shift.id)}
                      className="hover:scale-110 transition-transform text-slate-300 hover:text-red-600"
                    >
                      <Trash2 size={16} />
                    </button>
                  </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div className="bg-slate-50/50 p-3 rounded-xl border border-slate-100 flex items-center gap-3 group hover:border-slate-200 transition-colors">
                  ...
                  <span className="text-[11px] font-black text-slate-600 uppercase tracking-tight truncate">
                    LDR:{' '}
                    {shift.leader
                      ? `${shift.leader.split(' ')[0]} ${shift.leader.split(' ')[1] || ''}`.trim()
                      : '---'}
                  </span>
                </div>
              ...
              <div className="relative pt-2">
                <div className="flex justify-between items-end mb-3">
                    ...
                    <div className="flex -space-x-3">
                      {shift.leader && (
                        <Avatar
                          size="sm"
                          alt={shift.leader}
                          fallback={shift.leader.substring(0, 2)}
                          className="ring-2 ring-white border border-amber-400 shadow-md"
                        />
                      )}
                      {shift.members
                        .filter(
                  ...
                  <div className="text-right">
                    <span className="text-[10px] font-black text-slate-400 block mb-0.5 uppercase tracking-widest">
                      Capacidade
                    </span>
                ...
                <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden shadow-inner flex">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, ((shift.members.filter((m) => m.status === 'CONFIRMED' && m.name.trim().toLowerCase() !== (shift.leader || '').trim().toLowerCase()).length + (shift.leader ? 1 : 0)) / shift.vacancies) * 100)}%`,
                    }}
                  />
                </div>
              </div>
              ...
              >
                {shift.members.some((m) => m.status === 'VOLUNTEER_PENDING') ? (
                  <>
                    <UserCheck size={18} className="mr-3" /> Resolver
                    Solicitação
                  </>
                ) : (
                  <>
      ...
      <Modal
        ...
        title="Prévia da Escala (Rodízio)"
        maxWidth="lg"
      >
        <div className="space-y-4">
          <div className="bg-blue-50 p-4 rounded-xl text-blue-800 text-sm flex items-start gap-3">
            ...
            <div>
              ...
              <p>
                O sistema selecionou os associados abaixo baseado no histórico
                de plantões (quem trabalhou menos tem prioridade). Revise antes
                de confirmar.
              </p>
            </div>
          </div>
      ...
      <Modal
        ...
        title="Gestão Detalhada do Plantão"
        maxWidth="3xl"
      >
        {selectedShift && (
          <div className="space-y-6 animate-in fade-in zoom-in-95 duration-300">
            ...
            <div className="bg-slate-900 rounded-3xl p-8 text-white overflow-hidden relative shadow-2xl">
              ...
              <div className="relative z-10">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <Badge
                      variant={getStatusColor(selectedShift.status)}
                      className="mb-3 font-black uppercase tracking-widest text-[10px] px-4 py-1.5 bg-white/10 text-white border-white/20"
                    >
                ...
                <div className="grid grid-cols-3 gap-6 pt-6 border-t border-white/10">
                  <div className="text-center group">
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1 group-hover:text-slate-300 transition-colors">
                      Vagas Totais
                    </p>
                    <p className="text-2xl font-black">
                      {selectedShift.vacancies}
                    </p>
                  </div>
                  <div className="text-center border-x border-white/10 px-4 group">
                    ...
                    <p className="text-2xl font-black text-emerald-400">
                      {selectedShift.members.filter(
                        (m) =>
                          m.status === 'CONFIRMED' &&
                          m.name.trim().toLowerCase() !==
                            (selectedShift.leader || '').trim().toLowerCase()
                      ).length + (selectedShift.leader ? 1 : 0)}
                    </p>
                  </div>
                  <div className="text-center group">
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1 group-hover:text-slate-300 transition-colors">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {selectedShift.leader && (
                <div className="p-5 bg-amber-50/50 border border-amber-200 rounded-2xl flex items-center justify-between group hover:bg-amber-100/50 transition-all duration-300 shadow-sm hover:shadow-md">
                  ...
                  <div className="bg-amber-600 text-white text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-widest shadow-lg shadow-amber-200">
                    Vaga 1
                  </div>
                </div>
              )}
            ...
            {(selectedShift.members.find((m) => m.userId === user.id)
              ?.status === 'PENDING' ||
              (user.name.trim().toLowerCase() ===
                (selectedShift.leader || '').trim().toLowerCase() &&
                selectedShift.status === 'AWAITING_CONFIRMATION' &&
              ...
              <div className="p-6 bg-brand-50 border-2 border-brand-200 rounded-3xl text-center shadow-xl shadow-brand-100 border-dashed scrollbar-hide">
                ...
                <div className="flex gap-3 justify-center">
                  <Button
                    variant="outline"
                    className="h-12 px-6 border-brand-200 text-brand-700 hover:bg-white rounded-xl font-black uppercase text-[10px] tracking-widest"
                    onClick={() => handleRespondSummon(selectedShift.id, false)}
                  >
                    Recusar
                  </Button>
                  <Button
                    className="h-12 px-8 bg-brand-600 hover:bg-brand-700 shadow-2xl shadow-brand-200 rounded-xl font-black uppercase text-[10px] tracking-widest"
                    onClick={() => handleRespondSummon(selectedShift.id, true)}
                  >
                    Confirmar Presença
                  </Button>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between px-2">
                  ...
                  <Badge
                    variant="success"
                    className="px-3 py-1 font-black shadow-sm"
                  >
                    {selectedShift.members.filter(
                      (m) =>
                        m.status === 'CONFIRMED' &&
                ...
                <div className="bg-white rounded-3xl border border-slate-100 divide-y divide-slate-50 overflow-hidden shadow-sm">
                  {selectedShift.leader && (
                    <div className="p-4 flex items-center justify-between bg-amber-50/30">
                      <div className="flex items-center gap-3">
                        <Avatar
                          alt={selectedShift.leader}
                          fallback={selectedShift.leader.substring(0, 2)}
                          size="md"
                          className="border-2 border-amber-400 p-0.5"
                        />
                        <div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between px-2">
                  <h4 className="text-[11px] font-black text-slate-900 uppercase tracking-[0.2em] flex items-center gap-2">
                    <Clock size={16} className="text-amber-500" /> Gerenciar
                    Solicitações
                  </h4>
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                    {
                      selectedShift.members.filter((m) =>
                        ['PENDING', 'VOLUNTEER_PENDING'].includes(m.status)
                      ).length
                    }{' '}
                    Pendentes
                  </span>
                ...
                <div className="bg-slate-50 rounded-3xl border border-slate-100 divide-y divide-slate-100 overflow-hidden shadow-inner min-h-[150px]">
                  {selectedShift.members
                    .filter((m) =>
                      ['PENDING', 'VOLUNTEER_PENDING', 'DECLINED'].includes(
                        m.status
                      )
                    )
                    .map((m, i) => (
                      <div
                        key={i}
                        className={`p-4 flex items-center justify-between group ${m.status === 'DECLINED' ? 'bg-rose-50/50' : 'hover:bg-white'} transition-all`}
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p
                              className={`text-sm font-black ${m.status === 'DECLINED' ? 'text-slate-400 line-through' : 'text-slate-800'}`}
                            >
                              {m.name}
                            </p>
                            <div className="flex items-center gap-2">
                              ...
                              {m.status === 'VOLUNTEER_PENDING' && (
                                <Badge
                                  variant="info"
                                  className="text-[8px] h-4 py-0 font-black bg-blue-100 text-blue-600 border-none"
                                >
                                  URGENTE
                                </Badge>
                              )}
                        ...
                        {user.role === UserRole.ADMIN &&
                          m.status === 'VOLUNTEER_PENDING' && (
                            <div className="flex gap-2 animate-in slide-in-from-right-2">
                              <button
                                onClick={() =>
                                  handleRejectVolunteer(selectedShift, m.userId)
                                }
                  ...
                  {selectedShift.members.filter((m) =>
                    ...
                    <div className="p-12 text-center text-slate-400 flex flex-col items-center">
                      ...
                      <p className="text-[11px] font-black uppercase tracking-widest">
                        Sem pendências para este plantão
                      </p>
                    </div>
                  )}
                </div>
            ...
            <div className="pt-8 border-t border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-6">
              ...
              <div className="flex gap-4 w-full sm:w-auto">
                ...
                {!canEdit &&
                  !selectedShift.members.some((m) => m.userId === user.id) &&
                  selectedShift.status !== 'CONFIRMED' && (
                    <Button
                    ...
                    >
                      <UserPlus size={20} className="mr-3" /> Solicitar
                      Inscrição
                    </Button>
                  )}
                ...
                {canEdit && selectedShift.status === 'CONFIRMED' && (
                  ...
                  >
                    <Shield size={20} className="mr-3 text-brand-400" />{' '}
                    Finalizar Plantão
                  </Button>
                )}
                <Button
                  variant="outline"
                  className="flex-1 sm:flex-none h-14 px-10 rounded-2xl font-black uppercase text-[11px] tracking-widest text-slate-400 hover:bg-slate-50 hover:text-slate-600 border-slate-200"
      ...
      <Modal
        ...
        title="➕ Lançar Novo Plantão Operacional"
        maxWidth="3xl"
      >
        <form
          onSubmit={handleCreateShift}
          className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300 scrollbar-hide p-1"
        >
          <div className="grid grid-cols-12 gap-x-4 gap-y-3">
            <div className="col-span-12">
              ...
              <Input
                ...
                value={newShift.team}
                onChange={(e) =>
                  setNewShift({ ...newShift, team: e.target.value })
                }
            ...
            <div className="col-span-12 md:col-span-3">
              ...
              <Input
                ...
                onChange={(e) =>
                  setNewShift({ ...newShift, fullDate: e.target.value })
                }
            ...
            <div className="col-span-12 md:col-span-3">
              ...
              <div className="flex items-center gap-1">
                <select
                  className="flex-1 h-10 rounded-xl border border-slate-200 bg-white px-1 text-[11px] font-black outline-none transition-all appearance-none text-center focus:ring-2 focus:ring-brand-500"
                  value={newShift.startTime?.split(':')[0] || '08'}
                  onChange={(e) =>
                    setNewShift({
                      ...newShift,
                      startTime: `${e.target.value}:${newShift.startTime?.split(':')[1] || '00'}`,
                    })
                  }
                >
                ...
                <select
                  className="flex-1 h-10 rounded-xl border border-slate-200 bg-white px-1 text-[11px] font-black outline-none transition-all appearance-none text-center focus:ring-2 focus:ring-brand-500"
                  value={newShift.startTime?.split(':')[1] || '00'}
                  onChange={(e) =>
                    setNewShift({
                      ...newShift,
                      startTime: `${newShift.startTime?.split(':')[0] || '08'}:${e.target.value}`,
                    })
                  }
                >
                  {['00', '15', '30', '45'].map((m) => (
            ...
            <div className="col-span-12 md:col-span-3">
              ...
              <div className="flex items-center gap-1">
                <select
                  className="flex-1 h-10 rounded-xl border border-slate-200 bg-white px-1 text-[11px] font-black outline-none transition-all appearance-none text-center focus:ring-2 focus:ring-brand-500"
                  value={newShift.endTime?.split(':')[0] || '20'}
                  onChange={(e) =>
                    setNewShift({
                      ...newShift,
                      endTime: `${e.target.value}:${newShift.endTime?.split(':')[1] || '00'}`,
                    })
                  }
                >
                  {Array.from({ length: 24 }).map((_, i) => (
                ...
                <select
                  className="flex-1 h-10 rounded-xl border border-slate-200 bg-white px-1 text-[11px] font-black outline-none transition-all appearance-none text-center focus:ring-2 focus:ring-brand-500"
                  value={newShift.endTime?.split(':')[1] || '00'}
                  onChange={(e) =>
                    setNewShift({
                      ...newShift,
                      endTime: `${newShift.endTime?.split(':')[0] || '20'}:${e.target.value}`,
                    })
                  }
                >
                  {['00', '15', '30', '45'].map((m) => (
            ...
            <div className="col-span-12 md:col-span-5">
              ...
              <select
                ...
                onChange={(e) =>
                  setNewShift({ ...newShift, leader: e.target.value })
                }
              >
            ...
            <div className="col-span-12 md:col-span-12">
              ...
              <Input
                ...
                onChange={(e) =>
                  setNewShift({ ...newShift, organizer: e.target.value })
                }
          ...
          <div className="flex gap-3 pt-3 border-t border-slate-100">
            ...
            <Button
              type="submit"
              className="flex-1 h-10 bg-slate-900 hover:bg-black text-white shadow-xl shadow-slate-200 font-black text-[10px] uppercase tracking-widest"
            >
      ...
      >
        <form
          ...
          className="space-y-4 animate-in fade-in scrollbar-hide p-1"
        >
          <div className="grid grid-cols-12 gap-x-4 gap-y-3">
            <div className="col-span-12">
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 ml-1">
              ...
              <Input
                ...
                onChange={(e) =>
                  setEditingShift({ ...editingShift, team: e.target.value })
                }
                required
                className="h-10 rounded-xl"
              />
            ...
            <div className="col-span-12 md:col-span-3">
              ...
              <Input
                type="date"
                value={editingShift.fullDate}
                onChange={(e) =>
                  setEditingShift({ ...editingShift, fullDate: e.target.value })
                }
                required
                className="h-10 rounded-xl px-2"
              />
            ...
            <div className="col-span-12 md:col-span-3">
              ...
              <div className="flex items-center gap-1">
                <select
                  className="flex-1 h-10 rounded-xl border border-slate-200 bg-white px-1 text-[11px] font-black outline-none transition-all appearance-none text-center focus:ring-2 focus:ring-brand-500"
                  value={editingShift.startTime?.split(':')[0] || '08'}
                  onChange={(e) =>
                    setEditingShift({
                      ...editingShift,
                      startTime: `${e.target.value}:${editingShift.startTime?.split(':')[1] || '00'}`,
                    })
                  }
                >
                  {Array.from({ length: 24 }).map((_, i) => (
                ...
                <select
                  className="flex-1 h-10 rounded-xl border border-slate-200 bg-white px-1 text-[11px] font-black outline-none transition-all appearance-none text-center focus:ring-2 focus:ring-brand-500"
                  value={editingShift.startTime?.split(':')[1] || '00'}
                  onChange={(e) =>
                    setEditingShift({
                      ...editingShift,
                      startTime: `${editingShift.startTime?.split(':')[0] || '08'}:${e.target.value}`,
                    })
                  }
                >
                  {['00', '15', '30', '45'].map((m) => (
            ...
            <div className="col-span-12 md:col-span-3">
              ...
              <div className="flex items-center gap-1">
                <select
                  className="flex-1 h-10 rounded-xl border border-slate-200 bg-white px-1 text-[11px] font-black outline-none transition-all appearance-none text-center focus:ring-2 focus:ring-brand-500"
                  value={editingShift.endTime?.split(':')[0] || '20'}
                  onChange={(e) =>
                    setEditingShift({
                      ...editingShift,
                      endTime: `${e.target.value}:${editingShift.endTime?.split(':')[1] || '00'}`,
                    })
                  }
                >
                  {Array.from({ length: 24 }).map((_, i) => (
                ...
                <select
                  className="flex-1 h-10 rounded-xl border border-slate-200 bg-white px-1 text-[11px] font-black outline-none transition-all appearance-none text-center focus:ring-2 focus:ring-brand-500"
                  value={editingShift.endTime?.split(':')[1] || '00'}
                  onChange={(e) =>
                    setEditingShift({
                      ...editingShift,
                      endTime: `${editingShift.endTime?.split(':')[0] || '20'}:${e.target.value}`,
                    })
                  }
                >
                  {['00', '15', '30', '45'].map((m) => (
            ...
            <div className="col-span-12 md:col-span-5">
              ...
              <select
                ...
                onChange={(e) =>
                  setEditingShift({ ...editingShift, leader: e.target.value })
                }
              >
                <option value="">Selecione...</option>
                {directors.map((d) => (
          ...
          <div className="flex gap-3 pt-3 border-t border-slate-100">
            ...
            <Button
              type="submit"
              className="flex-1 h-10 bg-brand-600 hover:bg-brand-700 text-white font-black text-[10px] uppercase tracking-widest shadow-lg shadow-brand-100"
            >
