...
export const scheduleService = {
  ...
  async create(shift: Omit<Shift, 'id' | 'day' | 'date'>): Promise<Shift> {
    ...
    if (newShift.leader && newShift.leader !== 'A definir') {
      const { data: targetDirector } = await supabase
        .from('profiles')
        .select('id')
        .eq('full_name', newShift.leader)
        .maybeSingle();
      if (targetDirector) {
  ...
  async generatePreview(shiftId: string): Promise<ShiftMember[]> {
    try {
      ...
      const currentMembers: ShiftMember[] = Array.isArray(
        shift.confirmed_members
      )
        ? shift.confirmed_members.map((m: any) =>
            typeof m === 'string'
              ? { userId: '', name: m, status: 'CONFIRMED' }
              : m
          )
        : [];
      ...
      if (lastShiftError && lastShiftError.code !== 'PGRST116')
        throw lastShiftError;
      ...
      if (lastShift && Array.isArray(lastShift.confirmed_members)) {
        lastShiftUserIds = lastShift.confirmed_members
          .filter(
            (m: any) => m.status === 'CONFIRMED' || m.status === 'FINISHED'
          )
          .map((m: any) => m.userId);
      }
      ...
      const candidates = associates.filter((a) => {
        ...
        const isLeaderByName =
          leaderName &&
          (fullNameLower.includes(leaderName) ||
            leaderName.includes(fullNameLower));
      ...
      const sortFairly = (list: any[]) => {
        return list.sort((a, b) => {
          ...
          const dateA = a.last_shift_date
            ? new Date(a.last_shift_date).getTime()
            : 0;
          const dateB = b.last_shift_date
            ? new Date(b.last_shift_date).getTime()
            : 0;
  ...
  async volunteer(
    ...
    // Notificar Admin
    const adminIds = await associatesService.getNotificationTargets([
      'ADMIN',
      'SECRETARY',
    ]);
    await notificationService.add({
  ...
  async finalizeShift(shiftId: string): Promise<void> {
    ...
    if (shift.status === 'FINISHED')
      throw new Error('Este plantão já foi finalizado.');

    const members: ShiftMember[] = shift.confirmed_members || [];

    // 2. Filter CONFIRMED members to update stats
    const participants = members.filter(
      (m) => m.status === 'CONFIRMED' && m.userId
    );
...
function mapToFrontend(row: any): Shift {
  ...
  const adjustedEndDate = endObj
    ? new Date(endObj.getTime() + userTimezoneOffset)
    : null;
  ...
  return {
    id: row.id,
    fullDate:
      row.start_time?.split('T')[0] || row.start_time?.split(' ')[0] || '',
    day: dayStr,
    date: dateStr,
    team: row.title,
