  ...
  async subscribeUser(userId: string, silent = false) {
    ...
    try {
      ...
      if (currentPermission === 'denied') {
        if (!silent)
          alert(
            'As notificações estão bloqueadas no seu navegador. Por favor, ative-as nas configurações do site.'
          );
        return null;
      }
      ...
      const VAPID_PUBLIC_KEY =
        'BD759cctMRGblSVMXW6VDWFD-Ep_FAkHoHFa_jhGyKeco4vmHI9PQ8tGpX7iFjwLmU-hU02cZekSv9n-1k3o7sw';
      ...
      if (!silent) {
        if (err.name === 'NotAllowedError') {
          alert(
            'As notificações foram bloqueadas. Por favor, ative-as nas configurações do seu navegador para este site.'
          );
        } else {
          alert(
            'Erro ao ativar notificações: ' +
              (err.message || 'Verifique sua conexão ou chaves VAPID.')
          );
        }
      }
      throw err;
