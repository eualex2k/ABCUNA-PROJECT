...
export const financialService = {
  ...
  async create(transaction: Omit<Transaction, 'id'>): Promise<Transaction> {
    ...
    if (error) {
      console.error(
        'Error creating transaction full obj:',
  ...
  async update(
    ...
    if (error) {
      console.error(
        'Error updating transaction full obj:',
        JSON.stringify(error, null, 2)
      );
  ...
  async deleteBulk(ids: string[]): Promise<void> {
    ...
    if (error) {
      console.error(
        'SERVER ERROR - deleting multiple transactions:',
        JSON.stringify(error, null, 2)
      );
      throw error;
    }
  ...
  async attachComprovante(transaction_id: string, filePath: string) {
    ...
    const { error } = await supabase.from('financial_comprovantes').insert({
      transaction_id,
      file_path: filePath,
      created_by: user.id,
    });
    ...
    await this.logAudit(transaction_id, 'UPLOAD_COMPROVANTE', {
      file_path: filePath,
    });
  },
  ...
  async deleteComprovante(comprovante: FinancialComprovante) {
    ...
    if (!data || data.length === 0) {
      throw new Error(
        'Registro não removido. Verifique se você tem permissão de Administrador.'
      );
    }
    ...
    if (storageError)
      console.error('Error removing from bucket:', storageError);

    await this.logAudit(comprovante.transaction_id, 'DELETE_COMPROVANTE', {
      file_path: comprovante.file_path,
    });
  },

  async getComprovantes(
    transaction_id: string
  ): Promise<FinancialComprovante[]> {
    const { data, error } = await supabase
      .from('financial_comprovantes')
      .select(
        `
  ...
  async getSignedUrl(filePath: string): Promise<string> {
    ...
    try {
      ...
      if (error || !data?.signedUrl) {
        console.warn(
          '[Storage] Erro ao gerar URL assinada, tentando URL pública:',
          error
        );
        ...
        if (!publicData?.publicUrl) {
          throw new Error(
            'Não foi possível gerar uma URL válida para este arquivo.'
          );
        }
...
function mapToFrontend(row: any): Transaction {
  return {
    ...
    has_comprovantes:
      row.financial_comprovantes && row.financial_comprovantes.length > 0,
...
function mapToDb(tx: Partial<Transaction>): any {
  ...
  if (tx.custom_recipient !== undefined)
    dbRow.custom_recipient = tx.custom_recipient;
  if (tx.notes !== undefined) dbRow.notes = tx.notes;
  if (tx.registration_id !== undefined)
    dbRow.registration_id = tx.registration_id;
  return dbRow;
}
