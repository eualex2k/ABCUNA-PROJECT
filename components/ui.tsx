...
export const Card: React.FC<{
  ...
  glass?: boolean;
  hover?: boolean;
}> = ({ children, className = '', onClick, glass = false, hover = true }) => (
  <div
    onClick={onClick}
    className={`
...
export const Button: React.FC<ButtonProps> = ({
  ...
  const variants = {
    ...
    outline:
      'border border-slate-300 text-slate-700 bg-white hover:bg-slate-50 focus:ring-brand-500',
...
export const Input: React.FC<InputProps> = ({
  ...
  const isNumericMask =
    props.mask && ['phone', 'cpf', 'cnpj', 'cep', 'cns'].includes(props.mask);
  ...
  const getMinWidth = (maskType?: string) => {
    switch (maskType) {
      case 'phone':
        return 'min-w-[200px]'; // (00) 00000-0000
      case 'cpf':
        return 'min-w-[180px]'; // 000.000.000-00
      case 'cnpj':
        return 'min-w-[220px]'; // 00.000.000/0000-00
      case 'cep':
        return 'min-w-[140px]'; // 00000-000
      case 'cns':
        return 'min-w-[240px]'; // 000 0000 0000 0000
      default:
        return '';
    }
  };
    ...
    <div className="w-full">
      ...
      <div className="relative">
        <input
          ref={inputRef}
          type={
            isDate ? 'text' : props.type || (isNumericMask ? 'tel' : 'text')
          }
...
const DatePickerModal: React.FC<DatePickerModalProps> = ({
  ...
  const [viewDate, setViewDate] = React.useState(
    new Date(getInitialDate().getFullYear(), getInitialDate().getMonth(), 1)
  );
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex flex-col">
            ...
            >
              ...
              <ChevronDown
                size={14}
                className={`transition-transform duration-300 ${view === 'MONTHS' ? 'rotate-180' : ''}`}
              />
            </button>
            <button
              type="button"
            ...
            >
              ...
              <ChevronDown
                size={10}
                className={`transition-transform duration-300 ${view === 'YEARS' ? 'rotate-180' : ''}`}
              />
            </button>
          </div>
          {view === 'DAYS' && (
            ...
            <div className="animate-in fade-in duration-300">
              <div className="grid grid-cols-7 gap-1 mb-2">
                {['D', 'S', 'T', 'Q', 'Q', 'S', 'S'].map((day, idx) => (
                  <div
                    key={idx}
                    className="h-10 w-11 flex items-center justify-center text-[10px] font-black text-slate-300 uppercase tracking-widest"
                  >
                    {day}
                  </div>
                ))}
...
export const Avatar: React.FC<{
  ...
  return (
    <div
      className={`${sizeClasses[size]} relative rounded-full overflow-hidden bg-slate-200 flex items-center justify-center text-slate-600 font-bold ${border ? 'border-2 border-white' : 'border border-white'} ${shadow ? 'shadow-md' : 'shadow-sm'} ring-1 ring-slate-100 flex-shrink-0 ${className}`}
    >
      {src ? (
...
export const StatCard: React.FC<{
  ...
  <Card
    className="p-4 sm:p-6 flex flex-col justify-between h-full hover-lift"
    glass={false}
  >
    {loading ? (
      ...
      <>
        <div className="flex justify-between items-start mb-4">
          ...
          {trend && (
            <span
              className={`text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-full ${trendUp ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-red-50 text-red-600 border border-red-100'}`}
            >
              {trend}
            </span>
          )}
