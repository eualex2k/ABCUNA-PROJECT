...
export const AssociateDetails: React.FC<AssociateDetailsProps> = ({
          ...
          <div className="flex justify-between items-center mb-3">
            ...
            {canEdit && (
              <Button
                variant="ghost"
                size="sm"
                className="h-7 text-[10px] font-black uppercase tracking-widest hover:bg-slate-200"
              >
                Editar
              </Button>
            )}
  ...
  if (tab === 'FINANCIAL') {
    ...
    const oldestOverdue =
      overduePayments.length > 0
        ? overduePayments[overduePayments.length - 1]
        : null;
      ...
      <div className="space-y-4 animate-in fade-in duration-300">
        <div className="flex items-center justify-between p-5 bg-slate-900 rounded-2xl text-white shadow-xl shadow-slate-200">
          ...
          <div className="flex items-center gap-4">
            ...
            <div
              className={`p-3 rounded-xl bg-white/10 ${overduePayments.length === 0 ? 'text-emerald-400' : 'text-rose-400'}`}
            >
              <DollarSign size={24} />
            </div>
          </div>
        ...
        <Card className="p-0 overflow-hidden border-slate-100 shadow-sm">
          <div className="px-5 py-3 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
              Histórico de Cobranças / Taxas
            </p>
            <Badge variant="neutral" className="font-bold">
              {associateFees.length} Lançamentos
            </Badge>
          </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {associateFees.length === 0 ? (
                  <tr>
                    ...
                    >
                      ...
                      <p className="text-xs font-medium italic">
                        Nenhuma cobrança encontrada para este perfil.
                      </p>
                    </td>
                  </tr>
                ) : (
                  associateFees.map((fee) => {
                    const isFeeOverdue =
                      fee.status === 'PENDING' &&
                      new Date(fee.date + 'T12:00:00') < new Date();
  ...
  if (tab === 'HISTORY') {
    ...
    const associateShifts = shifts.filter(
      (s) =>
        s.members.some(
          (m) => m.name === associate.name && m.status === 'CONFIRMED'
        ) &&
        new Date(s.fullDate) < today &&
        s.status === 'CONFIRMED'
    );

    const totalRemuneration = associateShifts.reduce(
      (sum, s) => sum + s.amount,
      0
    );
        ...
        <div className="space-y-4">
          <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Clock size={16} /> Detalhamento Operacional
            </div>
            ...
            {associateShifts.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-slate-400 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                ...
                <p className="text-xs italic font-bold">
                  Nenhum histórico operacional sincronizado.
                </p>
              </div>
            ) : (
              associateShifts
                ...
                .map((shift, i) => (
                  <div
                    key={i}
                    className="flex gap-4 p-4 bg-white hover:bg-slate-50 rounded-2xl transition-all border border-slate-100 group shadow-sm hover:shadow-md"
                  >
                    <div className="flex flex-col items-center justify-center bg-slate-900 rounded-xl px-4 py-2 h-fit group-hover:bg-brand-600 transition-colors">
                      <span className="text-[10px] font-black text-white/60 uppercase leading-tight">
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between items-start">
                        <div>
                          <h5 className="font-black text-slate-900 uppercase text-sm tracking-tight">
                            {shift.team}
                          </h5>
