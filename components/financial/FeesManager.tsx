        ...
        <div className="flex flex-wrap items-center gap-3 p-3 bg-brand-50/50 rounded-xl border border-brand-100 animate-in slide-in-from-top-2 duration-300">
          <div className="flex items-center gap-2">
            <input
              ...
              checked={
                ...
                selectedFeeIds.length === unpaidFees.length
              }
            ...
            <label
              htmlFor="select-all"
              className="text-[11px] font-black text-brand-900 uppercase tracking-wider cursor-pointer"
            >
          ...
          <div className="ml-auto flex items-center gap-2">
            <span className="text-[10px] font-bold text-slate-500 mr-2">
              {selectedFeeIds.length} selecionadas
            </span>
            <Button
              size="sm"
      ...
      <div className="max-h-[500px] overflow-y-auto pr-1 space-y-3 custom-scrollbar">
        {loading ? (
          ...
          unpaidFees.map((fee) => (
            <div
              key={fee.id}
              className={`group p-4 rounded-xl border transition-all duration-300 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 ${
              ...
              <div className="flex items-center gap-4 flex-1">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <Badge
                      variant={fee.status === 'LATE' ? 'danger' : 'warning'}
                      className="text-[8px] font-bold px-1.5 py-0.5 uppercase tracking-wider"
                    >
          ...
          <div className="text-center py-20 bg-white rounded-[2rem] border border-slate-100 shadow-premium">
            ...
            <p className="text-slate-400 font-medium text-sm">
              Nenhuma mensalidade pendente encontrada nesta visualização.
            </p>
          </div>
        )}
      </div>
