      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h3 className="text-lg font-bold text-slate-900">
            Taxas de Inscrição 2026
          </h3>
          <p className="text-sm text-slate-500">
            Gestão de novos inscritos e pagamentos iniciais.
          </p>
        </div>
        ...
        {registrations.length > 0 ? (
          registrations.map((reg) => {
            ...
            return (
              <Card
                key={reg.id}
                className="p-5 flex flex-col justify-between hover-lift"
              >
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 py-3 border-y border-slate-100 bg-slate-50/30 -mx-5 px-5">
                    ...
                    <div className="text-right">
                      ...
                      <p
                        className={`text-base font-black tracking-tighter ${remaining > 0 ? 'text-red-600' : 'text-slate-400'}`}
                      >
                ...
                <div className="flex gap-2 mt-6">
                  {remaining > 0 && (
                    <Button
                      ...
                      onClick={() => onPay(reg)}
                    >
                      <CreditCard size={14} className="mr-1.5" /> Receber
                      Pagamento
                    </Button>
                  )}
                  <Button
          ...
          <div className="col-span-full py-12 text-center bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
            ...
            <p className="text-slate-500 text-sm">
              Adicione um novo inscrito para começar o controle.
            </p>
          </div>
        )}
      </div>
