...
const limitadorGlobal = rateLimit({
  ...
  message: {
    sucesso: false,
    mensagem:
      'O sistema detectou um volume elevado de comandos operacionais. Por favor, aguarde alguns minutos antes de fazer novas requisições.',
  },
});
...
app.get('/status', (_req, res) => {
  res.status(200).json({
    sucesso: true,
    mensagem:
      'Servidor administrativo operacional da ABCUNA ativo e respondendo.',
...
app.use(
  (
    ...
    console.error(
      '❌ Exceção interna crítica detectada no pipeline do servidor:',
      err
    );
    res.status(500).json({
      sucesso: false,
      mensagem:
        'Desculpe, ocorreu uma instabilidade interna crítica nos nossos servidores de API ao processar a operação.',
    });
  }
);
...
app.listen(porta, () => {
  ...
  console.log(
    `🔒 Modo Somente Leitura (Read-Only): ${env.READ_ONLY_MODE ? 'ATIVO (Escritas bloqueadas)' : 'INATIVO (Ações de persistência liberadas)'}`
  );
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
});
